...
Il campo elettrico ha un verso ben preciso, che dipende dal segno della carica che lo genera.
...
\begin{tikzpicture}[scale=1]
  % Piastre
  \draw[thick, fill=red!10] (-3,3) -- (3,3) -- (3,3.15) -- (-3,3.15) -- cycle;
  \draw[thick, fill=blue!10] (-3,-3) -- (3,-3) -- (3,-3.15) -- (-3,-3.15) -- cycle;
  \node[right] at (3.2,3.07) {\(+\)};
  \node[right] at (3.2,-3.07) {\(-\)};
...
\begin{itemize}[leftmargin=1cm]
  \item un \textbf{generatore di tensione variabile} applica ai capi del provino una tensione \(V\), che può essere cambiata a piacere;
...
\begin{tikzpicture}[scale=1]
  ...
  % Generatore (lato sinistro)
  \generatore[0]{0}{1.5}{\(\mathcal{E}\)}

  % Provino cilindrico (lato destro) - disegnato come un rettangolo allungato
  \draw[thick, fill=gray!20] (5.7,1.0) rectangle (6.3,2.0);
  \node[left] at (5.7,1.5) {provino};

  % Amperometro in serie (lato superiore)
  \node[draw, circle, thick, fill=white, inner sep=3pt] at (3,3) {A};

  % Voltmetro in parallelo al provino
  \draw[thick] (6,3) -- (7.5,3);
  \draw[thick] (6,0) -- (7.5,0);
  \draw[thick] (7.5,0) -- (7.5,3);
  \node[draw, circle, thick, fill=white, inner sep=3pt] at (7.5,1.5) {V};

  % Etichette
  \node[above] at (3,3.3) {amperometro};
  \node[right] at (7.9,1.5) {voltmetro};
\end{tikzpicture}
\end{figure}
...
Il rapporto \(V/I\) è quindi costante per quel provino:
\[
\frac{V}{I} = \text{costante}.
\]
Questa costante dipende dal provino, non dalla tensione applicata. Ohm la chiamò \textbf{resistenza elettrica} e la indicò con \(R\):
\[
\frac{V}{I} = R.
\]
...
Quindi
\[
R = 270 \times 10^1\,[\Omega] = 2700\,[\Omega] = 2{,}7\,[\text{k}\Omega],
\]
con tolleranza \(\pm 1\%\).
\end{esempio}
