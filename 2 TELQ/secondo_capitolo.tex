\chapter{Teoria dei circuiti elettrici}
...
\begin{esempio}
\textbf{Circuito con un solo anello.} Si consideri un circuito formato da una pila con forza elettromotrice \(V = 12\,[\text{V}]\) e due resistori in serie, \(R_1 = 4\,[\Omega]\) e \(R_2 = 2\,[\Omega]\). Calcolare la corrente che circola nel circuito.

\smallskip
\textbf{Soluzione.} Il circuito ha una sola maglia. Scegliamo come verso di percorrenza quello orario. Percorrendo la maglia:

\begin{itemize}[leftmargin=1cm]
  \item attraverso la pila si passa dal polo negativo al polo positivo: guadagno \(+V\);
...
Da cui
\[
I = \frac{V}{R_1 + R_2} = \frac{12}{4 + 2} = 2\,[\text{A}].
\]
\end{esempio}
...
\begin{itemize}[leftmargin=1cm]
  \item due generatori \(V_1 = 12\,[\text{V}]\) e \(V_2 = 6\,[\text{V}]\);
  \item due resistori, \(R_1 = 3\,[\Omega]\) e \(R_2 = 1\,[\Omega]\).
\end{itemize}
I due generatori sono disposti in modo che \(V_1\) favorisca la corrente nel verso orario, mentre \(V_2\) si opponga. Calcolare la corrente \(I\) che circola nella maglia.
...
\begin{itemize}[leftmargin=1cm]
  \item attraverso \(V_1\) si passa dal polo negativo al polo positivo: guadagno \(+V_1\);
  \item attraverso \(R_1\) e \(R_2\): caduta \(V_1 = -R_1 I\) e \(V_2 = -R_2 I\);
  \item attraverso \(V_2\) si passa dal polo positivo al polo negativo: guadagno \(-V_2\);
\end{itemize}

La legge di Kirchhoff alle Tensioni (LKT) fornisce:
\[
V_1 - V_1 - V_2 - V_2 = 0 \qquad\Longrightarrow\qquad
...
\begin{tikzpicture}[scale=1]
  ...
  \draw[thick] (0,0) -- (0,1.2);
  \generatore[label = $V_{in}$, label dx = 0.8]{0}{2}
  \draw[thick] (0,2.8) -- (0,4);
...
\section{Resistenza equivalente}

\begin{concetto}
Due o più resistori possono essere collegati tra loro in due modi fondamentali: in \textbf{serie} o in \textbf{parallelo}. Da queste due configurazioni si ricava la \textbf{resistenza equivalente}, cioè il valore di un unico resistore che si comporta come l'insieme dei resistori considerati.
\end{concetto}

\subsection{Il collegamento in serie}

\begin{concetto}
Due o più resistori sono collegati \textbf{in serie} quando sono disposti uno dopo l'altro, in modo che la \textbf{stessa corrente} li attraversi tutti. Non ci sono diramazioni tra un resistore e il successivo.
\end{concetto}

\begin{center}
\begin{tikzpicture}[scale=1]
  \draw[thick] (0,0) -- (0.5,0);
  \resistoreANSI[label=$R_1$, label dy=0.5]{1.5}{0}
  \draw[thick] (2.5,0) -- (3,0);
  \resistoreANSI[label=$R_2$, label dy=0.5]{4}{0}
  \draw[thick] (5,0) -- (5.5,0);
  \resistoreANSI[label=$R_3$, label dy=0.5]{6.5}{0}
  \draw[thick] (7.5,0) -- (8,0);

  \draw[->, thick, blue] (0.1,0.4) -- (0.7,0.4) node[midway,above] {$I$};
  \draw[->, thick, blue] (7.4,0.4) -- (8,0.4) node[midway,above] {$I$};
\end{tikzpicture}
\end{center}

Applicando la legge di Kirchhoff alle tensioni all'unica maglia, la tensione totale è la somma delle tensioni sui singoli resistori:
\[
V = V_1 + V_2 + V_3 = R_1 I + R_2 I + R_3 I = (R_1 + R_2 + R_3) I.
\]
Dividendo per \(I\):
\[
\boxed{
...
\begin{concetto}
La resistenza equivalente di più resistori in serie è la \textbf{somma} delle singole resistenze. È quindi \textbf{sempre maggiore} della più grande delle resistenze collegate.
\end{concetto}

\subsection{Il collegamento in parallelo}

\begin{concetto}
Due o più resistori sono collegati \textbf{in parallelo} quando sono connessi tra gli stessi due nodi, in modo che ai loro capi ci sia la \textbf{stessa tensione}. La corrente totale si ripartisce tra i vari rami.
\end{concetto}
...
\begin{tikzpicture}[scale=1]
  ...
  \resistoreANSI[rot=90, label=$R_1$, label dx=0.5, label dy=0]{1}{1.5}
  \draw[thick] (1,0.51) -- (1,0);

  \draw[thick] (3,3) -- (3,2.5);
  \resistoreANSI[rot=90, label=$R_2$, label dx=0.5, label dy=0]{3}{1.5}
  \draw[thick] (3,0.51) -- (3,0);

  \draw[thick] (5,3) -- (5,2.5);
  \resistoreANSI[rot=90, label=$R_3$, label dx=0.5, label dy=0]{5}{1.5}
  \draw[thick] (5,0.51) -- (5,0);

  \filldraw (1,3) circle (0.05);
  \filldraw (3,3) circle (0.05);
  \filldraw (5,3) circle (0.05);
  \filldraw (1,0) circle (0.05);
  \filldraw (3,0) circle (0.05);
  \filldraw (5,0) circle (0.05);

  \draw[->, thick, blue] (0.2,3.3) -- (0.8,3.3) node[midway,above] {$I$};
  \draw[->, thick, red] (1,2.9) -- (1,2.3) node[midway,left] {$I_1$};
  \draw[->, thick, red] (3, 2.9) -- (3,2.3) node[midway,left] {$I_2$};
  \draw[->, thick, red] (5, 2.9) -- (5,2.3) node[midway, left] {$I_3$};
\end{tikzpicture}
\end{center}

Applicando la legge di Kirchhoff alle correnti al nodo superiore, la corrente totale è la somma delle correnti nei singoli rami:
\[
...
\[
\boxed{
\frac{1}{R_{eq}} = \frac{1}{R_1} + \frac{1}{R_2} + \frac{1}{R_3} + \dots \quad \Longrightarrow \quad R_{eq} = \frac{1}{\frac{1}{R_1} + \frac{1}{R_2} + \frac{1}{R_3} + \dots}
}
\]

\begin{concetto}
La resistenza equivalente di più resistori in parallelo è \textbf{sempre minore} della più piccola delle resistenze collegate. Aggiungere un ramo in parallelo offre alla corrente un percorso in più, quindi la resistenza complessiva diminuisce.
\end{concetto}


Per due soli resistori in parallelo la formula si semplifica:
\[
\boxed{
R_{eq} = \frac{1}{\frac{1}{R_1}+\frac{1}{R_2}} = \frac{R_1 \cdot R_2}{R_1 + R_2}
}
\]

\begin{esempio}
\textbf{Due resistori in parallelo.} Due resistori \(R_1 = 100\,\Omega\) e \(R_2 = 150\,\Omega\) sono in parallelo. La resistenza equivalente è:
\[
R_{eq} = \frac{100 \cdot 150}{100 + 150} = \frac{15000}{250} = 60\,\Omega.
\]
\(60\,\Omega\) è minore sia di \(100\,\Omega\) sia di \(150\,\Omega\), come deve essere.
\end{esempio}

\subsection{Serie e parallelo combinati}

Molti circuiti contengono resistori collegati in modo misto. Per calcolare la resistenza equivalente si procede \textbf{per riduzioni successive}: si individuano le parti puramente in serie o puramente in parallelo, si sostituiscono con la loro resistenza equivalente, e si ripete fino a ottenere un unico resistore.

\begin{esempio}
\textbf{Circuito misto.} \(R_1 = 10\,\Omega\) in serie con il parallelo di \(R_2 = 20\,\Omega\) e \(R_3 = 30\,\Omega\).

\begin{center}
\begin{tikzpicture}[scale=1]
  \draw[thick] (0,1.5) -- (0.5,1.5);
  \resistoreANSI[label=$R_1$, label dy=0.5]{1.5}{1.5}
  \draw[thick] (2.5,1.5) -- (3,1.5);

  \filldraw (3,1.5) circle (0.05);
  \filldraw (5,1.5) circle (0.05);

  \draw[thick] (3,1.5) -- (3,2.5);
  \resistoreANSI[rot=0, label=$R_2$, label dx=0, label dy=-0.45]{4}{2.5}
  \draw[thick] (5,2.5) -- (5,1.5);

  \draw[thick] (3,1.5) -- (3,0.5);
  \resistoreANSI[rot=0, label=$R_3$, label dx=0, label dy=0.45]{4}{0.5}
...
Prima il parallelo:
\[
R_{23} = \frac{R_2 \cdot R_3}{R_2 + R_3} = \frac{20 \cdot 30}{20 + 30} = 12\,\Omega.
\]

\hrule
\vspace{0.3cm}
Si procede ridisegnando il circuito, sostituendo il parallelo $R_2 \parallel R_3$ con il resistore equivalente $R_{23}$:

\begin{center}
\begin{tikzpicture}[scale=1]
  \draw[thick] (0,1.5) -- (0.5,1.5);
  \resistoreANSI[label=$R_1$, label dy=0.5]{1.5}{1.5}
  \resistoreANSI[label=$R_{23}$, label dy=0.5]{3.5}{1.5}
...
\begin{attenzione}
Per riconoscere se due resistori sono in serie o in parallelo bisogna guardare la \textbf{topologia del circuito}, non la loro posizione nel disegno. Due resistori sono in serie se sono percorsi dalla stessa corrente (non c'è alcun nodo tra loro); sono in parallelo se sono collegati tra gli stessi due nodi.
\end{attenzione}
...
\begin{esercizio}
\textbf{Esercizio svolto.} In una maglia sono presenti una pila con \(V = 9\,[\text{V}]\) e tre resistori in serie, \(R_1 = 1\,[\Omega]\), \(R_2 = 2\,[\Omega]\), \(R_3 = 3\,[\Omega]\). Calcolare la corrente che circola.

\smallskip
\textbf{Soluzione.} Applichiamo la seconda legge di Kirchhoff scegliendo come verso di percorrenza quello orario:
\[
V - R_1 I - R_2 I - R_3 I = 0.
\]
Da cui
\[
I = \frac{V}{R_1 + R_2 + R_3} = \frac{9}{1 + 2 + 3} = 1{,}5\,[\text{A}].
\]
\end{esercizio}
...
\begin{enumerate}[leftmargin=1cm]
  ...
  \item In una maglia ci sono due pile in serie, \(V_1 = 4{,}5\,[\text{V}]\) e \(V_2 = 1{,}5\,[\text{V}]\), e un resistore \(R = 3\,[\Omega]\). Calcolare la corrente.
